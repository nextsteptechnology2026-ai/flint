...
- **Vista previa de Markdown** (`Ctrl+E`): cambia el área de texto por el documento renderizado — sin `#`, sin `**`, con viñetas de verdad, citas con su barra al margen, cercos de código en un recuadro y **resaltados con tree-sitter según el lenguaje que declaren**, y links subrayados con su URL al lado. Los títulos son el caso interesante: en una terminal todas las letras miden igual, así que no hay forma de hacer un `h1` "más grande" — la jerarquía se da con sangría (dos espacios por nivel, cortada en el cuarto para no irse contra el borde) y con una raya debajo de los dos primeros niveles. Es de solo lectura sobre el mismo buffer: editar sigue siendo sobre el texto fuente, que es lo que uno quiere en un `.md` donde la sintaxis *es* el contenido. Se descartó el panel partido (editor y preview lado a lado): en 80 columnas deja 38 de cada lado y ninguna de las dos mitades se lee bien. El párrafo se reflowea al ancho de la ventana, con corte entre palabras y sin partir una palabra más larga que el ancho (una URL cortada no se puede volver a pegar con la vista). El render se cachea contra la versión del buffer y el ancho de pantalla: rehacerlo en cada frame —parsear el documento y compilar las consultas de tree-sitter de cada cerco— sería carísimo comparado con dibujar. Probado a mano: los estilos leyendo ANSI crudo, el reflow cambiando el ancho de la terminal, el desplazamiento con ↑↓/PgUp/PgDn/Home/End, y que escribir una letra no edite nada (la barra de título no enciende su marca de "sin guardar"). Siete tests cubren el render.
- **Markdown resaltado como texto, no como código**: `.md` y `.markdown` usan tree-sitter, con una particularidad: Markdown se parsea en dos gramáticas, una para los bloques (títulos, listas, citas, cercos de código) y otra para lo de adentro de una línea (`**negrita**`, `*cursiva*`, `` `código` ``, links). Flint corre las dos explícitamente —el árbol de bloques dice dónde hay tramos `inline` y esos se vuelven a analizar con la gramática inline—, en vez de usar el mecanismo de inyecciones de `tree-sitter-highlight`: ahí la capa inyectada nunca llegaba a emitir un evento, el callback se llamaba y devolvía la gramática correcta pero el resultado no aparecía en el flujo; dos pasadas a mano son treinta líneas legibles y probables. Además, las categorías de texto con formato no se distinguen por color sino por **forma**: un título va en negrita, `**negrita**` en negrita de verdad, `*cursiva*` en cursiva y un link subrayado — poner color donde va negrita sería traducir mal, la terminal ya sabe hacer negrita. No agrega ninguna clave al `theme.toml`: reusa colores que el tema ya define. Probado a mano leyendo los códigos ANSI crudos (`\e[1m`, `\e[3m`, `\e[4m`), no la captura de pantalla, y con cinco tests. De yapa, la selección estructural (`n`) ahora funciona en Markdown: seleccionar un párrafo y subir a la sección.
...
- **Ajuste de línea**: `Ctrl+L` (o "Alternar ajuste de línea" en la paleta) alterna entre scroll horizontal (el comportamiento de siempre, sigue siendo el default) y partir las líneas largas en varias filas de pantalla. La barra de título marca `[wrap]` cuando está activo. El número de línea y la marca de diagnóstico solo aparecen en la primera fila visual de cada línea; las siguientes son continuación, sin numerar (como la mayoría de los editores con wrap). El corte es por carácter, no por palabra — más simple y predecible que buscar el espacio más cercano, y es lo mismo que hace `nano` por defecto. Eso sí, nunca parte un carácter al medio: si el siguiente es ancho (CJK, emoji) y no entra entero, la fila termina antes y baja completo a la siguiente. Los cortes los calcula `wrap_row_starts` en un solo lugar, y de ahí los toman por igual el dibujo, el cálculo de scroll y los clics del mouse — si cada uno los recalculara a su manera, un clic caería en un carácter distinto del que se ve. El mouse mapea clics correctamente incluso en filas de continuación. Una sola línea más alta que toda la pantalla también se puede recorrer: el tope de la pantalla no es una línea sino una fila visual —el par (`row_offset`, `row_sub_offset`)—, así que el scroll entra *dentro* de la línea en vez de saltarla entera. Cuando la primera fila visible es continuación, va sin numerar, igual que cualquier otra. Probado a mano: activar wrap con líneas de ~150 caracteres en una terminal de 60 columnas, moverse con las flechas a través de las filas partidas, hacer scroll más allá de lo visible, clickear con el mouse en una fila de continuación (con una secuencia SGR cruda) y confirmar que el texto se insertó exactamente ahí, y volver a desactivarlo sin dejar rastros.
...
- Resaltado de sintaxis: ya no recorre el archivo entero en cada tecla. Reusa el árbol de la pasada anterior (`Tree::edit` + reparseo incremental), vuelve a consultar solo el tramo que tree-sitter marca como cambiado, y conserva el resaltado de las líneas que no se tocaron. Una tecla cuesta 0,4 ms en un archivo de 100 KB y 3,5 ms en uno de 3 MB, contra 57 ms y 1,16 s antes; por eso tampoco hace falta ya el margen de espera de 120 ms, y los colores se actualizan mientras escribís. La *primera* pasada, al abrir el archivo, sigue siendo proporcional al tamaño, pero desde 256 KB corre en otro hilo: primero se colorea lo que está en pantalla y después el resto, y mientras tanto el editor responde. Un archivo de 3 MB se ve a los ~40 ms (antes 1,8 s) y con color a los ~0,6 s.
